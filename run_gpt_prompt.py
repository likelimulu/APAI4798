...
class LLM_Output_Exception(Exception):  #用来检测__func_clean_up的错误
    pass
...
def run_gpt_prompt_wake_up_hour(persona, test_input=None, verbose=False): 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    cr = int(gpt_response.strip().split(":")[0])
    return cr
...
def run_gpt_prompt_daily_plan(persona, 
  ...
  def create_prompt_input(persona, wake_up_hour, test_input=None):
    ...
    prompt_input += [f"{str(wake_up_hour)}:00"]
    return prompt_input


  def __func_clean_up(gpt_response, prompt=""):    
    if type(gpt_response) != type([]):
      try:
        gpt_response = eval(gpt_response)
      except:
        print("Not list output from run_gpt_prompt_daily_plan()")
        print("Output is:", gpt_response)
        raise Exception("Not list output from run_gpt_prompt_daily_plan()")
  ...
  def get_fail_safe(): 
    fs = ['wake up at 6:00', 
          ...
          'read a book from 8:00 to 12:00', 
          'have lunch at 12:00', 
          'take a nap from 13:00 to 14:00', 
          'relax and watch TV from 19:00 to 20:00', 
          'go to bed at 23:00'] 
    return fs
...
def run_gpt_prompt_generate_hourly_schedule(persona, 
  ...
  def create_prompt_input(persona, 
    ...
    # schedule_format = f"""Please generate the hourly schedule for {persona.scratch.get_str_firstname()} according to the background information and intended hourly breakdown for current date, from {hour_str[0]} to {hour_str[-1]}."""
    requirement += f"(1)(Important) Please generate the hourly schedule for {persona.scratch.get_str_firstname()} according to the background information and intended hourly breakdown for current date, from {hour_str[0]} to {hour_str[-1]}.\n"
    requirement += "(2)(Important) Each activity is enclosed inside a pair of double quotes.\n"
    requirement += "(3)(Important) You only need to give the Acitivity corresponding to each hour in a list.\n"
    requirement += f"(4)(Important) The number of all activities(the length of the list) must be {len(hour_str)}, which is just the number of hours.\n"
    requirement += f"(5)If {persona.scratch.get_str_firstname()} performs the exact same behavior in consecutive time periods, please use the same words to describe the Activity.\n"
    requirement += "(6)If the behavior is different in consecutive time periods, just use different words to describe the Acitivity.\n"
    requirement += f"(7)It is best for {persona.scratch.get_str_firstname()} to do only one thing rather than multiple things within each hour."
    # for i in hour_str: 
    ...
    prompt_ending = "Example output format:\n" + """["00:00-sleeping", "01:00-sleeping", "02:00-sleeping", "03:00-sleeping", "04:00-sleeping", "05:00-sleeping", "06:00-waking up", "07:00-completing the morning routine", ..., "23:00-sleeping"]"""
  ...
  def __func_clean_up(gpt_response, prompt=""):
    cr = []
    try:
      cr = json.loads(gpt_response)
      cr = [item.split("-")[1] for item in cr]
    except:
      if type(gpt_response) == type([]):
        cr = [item.split("-")[1] for item in gpt_response]
  ...
  output = safe_generate_response(prompt, gpt_param, 5, fail_safe,
                                   __func_validate, __func_clean_up)
  # try:
  #   activity = json.loads(output)  #测试一下这里的输出是什么
  # except:
  #   return False, False
...
def run_gpt_prompt_task_decomp(persona, 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    try:
      ...
      # actions = [(action[0].lower() + action[1:]) for action in actions]

      # durations = [line.split("duration in minutes")[1] for line in lines]
      # durations = [duration.split(":")[1] for duration in durations]
      # durations = [duration.split(",")[0] for duration in durations]
      ...
      actions = [action.split("(")[0] for action in gpt_response]
      durations = [int(action.split("(")[1].split(",")[0]) for action in gpt_response]
      ...
      for i in range(len(durations)):
        if time_overflow == True:
          durations[i] = 0
        if total_true_min + durations[i]> total_expected_min:
          durations[i] = (total_expected_min - total_true_min)
          time_overflow = True
        if i == len(durations)-1 and total_true_min + durations[i] < total_expected_min:
          durations[i] = (total_expected_min - total_true_min)
        total_true_min += durations[i]

      cr = list(zip(actions, durations))
      return cr
      
    except Exception as e:
      print("-"*50)
      print("This is a LLM generation exception in function run_gpt_prompt_task_decomp()")
      print("")
      print("The gpt_response is:", gpt_response)
      print("")
      print("The Exception is:", str(e))
      print("-"*50)
      raise LLM_Output_Exception("This is a LLM generation exception")
  ...
  #     if task[-1] == ".": 
  #       task = task[:-1]
  #     duration = int(k[1].split(",")[0].strip())  #易报错行
  #     cr += [[task, duration]]
...
def run_gpt_prompt_action_ville(action_description, 
                                persona, 
                                maze, 
                                test_input=None, 
                                verbose=False):
  def create_prompt_input(action_description, persona, maze, test_input=None): 
    ...
    prompt_input += ['"' + maze.access_tile(persona.scratch.curr_tile)['ville'] + '"']
    x = f"{act_world}:{maze.access_tile(persona.scratch.curr_tile)['ville']}"
    prompt_input += [persona.s_mem.get_str_accessible_ville_sectors(x)]

    accessible_ville_str = persona.s_mem.get_str_accessible_villes(act_world)
    prompt_input += [accessible_ville_str]

    prompt_input += [persona.scratch.get_str_name()]
    prompt_input += [action_description]

    home_position = persona.scratch.get_living_area()
    home_position = home_position.split(":")
    home_position = home_position[::-1] #倒转地点
    home_position = ['"' + i + '"'for i in home_position]
    home_position[0] = "area:" + home_position[0]
    home_position[1] = "sector:" + home_position[1]
    home_position[2] = "village:" + home_position[2]
    home_position = home_position[0:3]
    # home_position[3] = "world:" + home_position[3] #这行加入世界信息的目前感觉没什么用
    prompt_input += [', '.join(home_position)]
  ...
  def __func_clean_up(gpt_response, prompt=""):
    try:
      if gpt_response[0] == "'" or gpt_response[0] == '"':
        gpt_response = gpt_response[1:]
      if gpt_response[-1] == "'" or gpt_response[-1] == '"':
        gpt_response = gpt_response[:-1]
      
      cleaned_response = gpt_response.strip()
      target_list = persona.s_mem.get_str_accessible_villes(f"{maze.access_tile(persona.scratch.curr_tile)['world']}").split(", ")
      target_list = [target[1:-1] for target in target_list]
      if cleaned_response in target_list:
        return cleaned_response
      else:
        raise Exception("Wrong ville output")
    except Exception as e:
      print("-"*50)
      print("This is a LLM generation exception in function run_gpt_prompt_action_ville()")
      print("")
      print("The gpt_response is:", gpt_response)
      print("The target list is", target_list)
      print("")
      print("The Exception is:", str(e))
      print("-"*50)
      raise LLM_Output_Exception("This is a LLM generation exception")
  ...
  gpt_param = {"engine": "text-davinci-002", "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v1/action_location_ville_v1.txt"
...
def run_gpt_prompt_action_sector(action_description, 
                                ...
                                test_input=None, 
                                verbose=False):
  def create_prompt_input(action_description, persona, maze, act_world, act_ville, test_input=None):   
    prompt_input = []

    prompt_input += [persona.scratch.get_str_name()]
    
    prompt_input += ['"' + act_ville + '"']

    x = f"{act_world}:{act_ville}"
    accessible_sector_str = persona.s_mem.get_str_accessible_ville_sectors(x)
  ...
  def __func_clean_up(gpt_response, prompt=""):
    try:
      ...
      cleaned_response = gpt_response.strip()
      target_list = persona.s_mem.get_str_accessible_ville_sectors(f"{act_world}:{act_ville}").split(", ")
      target_list = [target[1:-1] for target in target_list]
      if cleaned_response in target_list or "random sector" in cleaned_response:
        return cleaned_response
      else:
        raise Exception("Wrong sector output")
    except Exception as e:
      print("-"*50)
      print("This is a LLM generation exception in function run_gpt_prompt_action_sector()")
      print("")
      print("The gpt_response is:", gpt_response)
      print("The target list is", target_list)
      print("")
  ...
  prompt_input = create_prompt_input(action_description, persona, maze, act_world, act_ville)
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_action_arena(action_description, 
                                persona, 
                                maze, act_world, act_ville, act_sector,
                                test_input=None, 
                                verbose=False):
  def create_prompt_input(action_description, persona, maze, act_world, act_ville, act_sector, test_input=None): 
    prompt_input = []
    ...
    x = f"{act_world}:{act_ville}:{act_sector}"
  ...
  def __func_clean_up(gpt_response, prompt=""):
    try:
      ...
      cleaned_response = gpt_response.strip()
      target_list = persona.s_mem.get_str_accessible_sector_arenas(f"{act_world}:{act_ville}:{act_sector}").split(", ")
      target_list = [target[1:-1] for target in target_list]  #去除前后的"号
      if cleaned_response in target_list or "random area" in cleaned_response:
        return cleaned_response
      else:
        raise Exception("Wrong area output")
    except Exception as e:
      print("-"*50)
      print("This is a LLM generation exception in function run_gpt_prompt_action_arena()")
      print("")
      print("The gpt_response is:", gpt_response)
      print("The target list is", target_list)
      print("")
  ...
  prompt_input = create_prompt_input(action_description, persona, maze, act_world, act_ville, act_sector)
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_action_game_object(action_description, 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    try:
      ...
      cleaned_response = gpt_response.strip()
      target_list = persona.s_mem.get_str_accessible_arena_game_objects(temp_address).split(", ")
      target_list = [target[1:-1] for target in target_list]
      if cleaned_response in target_list or "random item" in cleaned_response:
        return cleaned_response
      else:
        raise Exception("Wrong object output")
    except Exception as e:
      print("-"*50)
      print("This is a LLM generation exception in function run_gpt_prompt_action_game_object()")
      print("")
      print("The gpt_response is:", gpt_response)
      print("The target list is", target_list)
      print("")
...
def run_gpt_prompt_pronunciatio(action_description, persona, verbose=False): 
  ...
  prompt = generate_prompt(prompt_input, prompt_template) #问题出在这里
  
  extra = "3.All emojis should be outputed in the same format as '🛁', '🧖‍♀️', '😋'.\n4.If the action is 'unrelated' or 'unused', use '❌'.\n5.If the action is 'in use' or 'used' or 'occupied', use '⌛'.\n"
  prompt = prompt.split("4.")
  prompt.insert(1, extra)
  prompt = "".join(prompt)
  
  fail_safe = get_fail_safe()

  output = safe_generate_response(prompt, gpt_param, 3, fail_safe, __func_validate, __func_clean_up)
...
def run_gpt_prompt_event_triple(action_description, persona, verbose=False): 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    try:
      ...
      cr = cr.split(")")[0]
      cr = cr.split(",")
      cr = [each_cr.strip() for each_cr in cr]
      cr = cr[1:]
      return cr
    except Exception as e:
      print("-"*50)
      print("This is a LLM generation exception in function run_gpt_prompt_event_triple()")
      print("")
      print("The gpt_response is:", gpt_response)
      print("")
      print("The Exception is:", str(e))
...
def run_gpt_prompt_act_obj_desc(act_game_object, act_desp, persona, verbose=False): 
  ...
  output = safe_generate_response(prompt, gpt_param, 5, fail_safe, __func_validate, __func_clean_up)  #把这个ChatGPT_safe_generate_response改为safe_generate_response，因为前者会加入"""导致引起bug(以及基于此原因，需要修改所有的ChatGPT_safe_generate_response为safe_generate_response)
...
def run_gpt_prompt_act_obj_event_triple(act_game_object, act_obj_desc, persona, verbose=False): 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    try:
      ...
      print("-"*50)
      print("This is a LLM generation exception in function run_gpt_prompt_act_obj_event_triple()")
      print("")
      print("The gpt_response is:", gpt_response)
      print("")
      print("The Exception is:", str(e))
...
def run_gpt_prompt_new_decomp_schedule(persona, 
  ...
  def create_prompt_input(persona, 
    ...
    for count, i in enumerate(truncated_act_dur): 
      schedule_part_1 += '"' + f'{for_time.strftime("%H:%M")} ~ {(for_time + datetime.timedelta(minutes=int(i[1]))).strftime("%H:%M")} -- ' + i[0] + '"'
    ...
    schedule_part_2 = "[\n"
    new_plan_hour = (for_time + datetime.timedelta(minutes=int(i[1]))).strftime("%H:%M")
    schedule_part_2 += '"' + new_plan_hour + ' ~ (wait to be fill in)",\n'
    schedule_part_2 += "...\n"
    schedule_part_2 += f'"(wait to be fill in) ~ {end_hour_str} -- (wait to be fill in)"\n'
    schedule_part_2 += "]"
  ...
  def __func_clean_up(gpt_response, prompt=""):
    ...
    new_schedule_part_2 = []
    if type(gpt_response) == type([]):
      new_schedule_part_2 = gpt_response
    else:
      try:
        new_schedule_part_2 = eval(gpt_response)
        if type(new_schedule_part_2) != type([]):
          raise Exception("Wrong new_schedule_part_2")
      except:
        print("Wrong new_schedule_part_2:")
        print(gpt_response)
        raise Exception("Wrong new_schedule_part_2")
    
    new_schedule_part_1 = []
    try:
      new_schedule_part_1 = eval(prompt.split("Revised schedule part 1:")[1].split("]")[0]+"]")
    except:
        print("Wrong new_schedule_part_1:")
        print(gpt_response)
        raise Exception("Wrong new_schedule_part_1")
    
    new_schedule = new_schedule_part_1 + new_schedule_part_2
...
def run_gpt_prompt_event_poignancy(persona, event_description, test_input=None, verbose=False): 
  ...
  def __chat_func_clean_up(gpt_response, prompt=""): ############
    try:
      ...
      raise Exception("Error event poignancy")
  ...
  fail_safe = get_fail_safe() ########
  output = safe_generate_response(prompt, gpt_param, 5, fail_safe, __chat_func_validate, __chat_func_clean_up)
...
def run_gpt_prompt_thought_poignancy(persona, event_description, test_input=None, verbose=False): 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    try:
      ...
      raise Exception("Error thought poignancy")
...
def run_gpt_prompt_chat_poignancy(persona, event_description, test_input=None, verbose=False): 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    try:
      ...
      raise Exception("Error chat poignancy")
...
def run_gpt_prompt_insight_and_guidance(persona, statements, n, test_input=None, verbose=False): 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    ...
    ret = dict()
    for insights in gpt_response: 
      thought = insights.split("--")[0].strip()
      evi_raw = insights.split("--")[1].strip()
...
def run_gpt_prompt_generate_next_convo_line(persona, interlocutor_desc, prev_convo, retrieved_summary, test_input=None, verbose=False): 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    try:
      ...
      print("-"*50)
      print("This is a LLM generation exception in function run_gpt_prompt_generate_next_convo_line()")
      print("")
      print("The gpt_response is:", gpt_response)
      print("")
      print("The Exception is:", str(e))
...
def run_gpt_generate_iterative_chat_utt(maze, init_persona, target_persona, retrieved, curr_context, curr_chat, test_input=None, verbose=False): 
  ...
  def __chat_func_clean_up(gpt_response, prompt=""): 
    try:
      ...
      cleaned_dict = dict()
      cleaned_dict["utterance"] = gpt_response[0]
      cleaned_dict["end"] = eval(gpt_response[1])

      return cleaned_dict
    
    except Exception as e:
      print("-"*50)
      print("This is a LLM generation exception in function run_gpt_generate_iterative_chat_utt()")
      print("")
      print("The gpt_response is:", gpt_response)
      print("")
      print("The Exception is:", str(e))
  ...
  prompt = generate_prompt(prompt_input, prompt_template)
  print("#####################################")
  print("run_gpt_generate_iterative_chat_utt: prompt")
  print (prompt)
  print("#####################################")
  fail_safe = get_fail_safe() 
  gpt_param = {"engine": "text-davinci-003", "max_tokens": 50, 
