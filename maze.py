...
class Maze: 
  def __init__(self, maze_name): 
    ...
    #用来标识不同村庄的,属于world的下级
    _vb = blocks_folder + "/ville_blocks.csv"
    vb_rows = read_file_to_list(_vb, header=False)
    vb_dict = dict()
    for i in vb_rows: vb_dict[i[0]] = i[-1]
    ...
    _vm = maze_folder + "/ville_maze.csv"
    ville_maze_raw = read_file_to_list(_vm, header=False)[0]
    ...
    for i in range(0, len(collision_maze_raw), meta_info["maze_width"]): 
      ...
      ville_maze += [ville_maze_raw[i:i+tw]]
    ...
    for i in range(self.maze_height): 
      ...
      for j in range(self.maze_width):
        ...
        tile_details["ville"] = ""
        if ville_maze[i][j] in vb_dict: 
          tile_details["ville"] = vb_dict[ville_maze[i][j]]
    ...
    for i in range(self.maze_height):
      for j in range(self.maze_width): 
        addresses = []
        if self.tiles[i][j]["world"]: 
          add = f'{self.tiles[i][j]["world"]}'
          addresses += [add]
        if self.tiles[i][j]["ville"]: 
          add = f'{self.tiles[i][j]["world"]}:'
          add += f'{self.tiles[i][j]["ville"]}'
          addresses += [add]
  ...
  def access_tile(self, tile): 
    ...
    EXAMPLE OUTPUT
      Given (58, 9), 
      self.tiles[9][58] = {'world': 'double studio', 'ville': 'Ville 1',
            'sector': 'double studio', 'arena': 'bedroom 2', 
            'game_object': 'bed', 'spawning_location': 'bedroom-2-a', 
            'collision': False,
            'events': {('double studio:double studio:bedroom 2:bed',
