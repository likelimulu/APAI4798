...
#以下代码是为了自动运行gemini_server.py
####################################################################################
import subprocess

def run_python_script_in_new_cmd(script_path):
    try:
        subprocess.Popen(['start', 'cmd', '/k', 'python', script_path], shell=True)
    except:
        print("could not run gemini_server.py")

# 调用函数在新的 cmd 窗口中运行脚本
run_python_script_in_new_cmd("gemini_server.py")
####################################################################################
...
class ReverieServer: 
  ...
  def start_server(self, int_counter): 
    ...
    while (True): 
      ...
      if check_if_file_exists(curr_env_file):   #获得？
        ...
        if env_retrieved: 
          ...
          #这里给它做成并行的(以生成每日计划为例，目前一个Gemini API处理1个prompt请求的时间为9秒，并行处理12个请求的时间为12秒，并行处理25个请求的时间为18秒，并行处理50个请求的时间为30秒)(并行情况下的执行速度很快)
          #(目前并行疑似有bug，会导致前端载入数据时死机, 或者可能是运行gemini时模拟小镇程序有时会失败的bug导致整体宕机
          #可以在这个函数的后部等位置加入终止代码，测试执行到哪一步发生宕机，并且观察执行流程，以确定宕机的具体原因
          def process_movement(persona_name, persona):
            next_tile, pronunciatio, description = persona.move(self.maze, self.personas, self.personas_tile[persona_name], self.curr_time)
          ...
          with concurrent.futures.ThreadPoolExecutor(max_workers=1000) as executor:
            futures = [executor.submit(process_movement, persona_name, persona) for persona_name, persona in self.personas.items()]
            ...
            for i in range(len(futures)):
              try:
                ...
                # 处理任务的异常信息，例如打印错误报告
                print(f"Error occurred for thred:", i)
                print(f"Error message: {str(e)}")
                sys.exit()

          #被上述并行代码替换的内容,需要保留着，因为还要修运行gemini时模拟小镇程序有时会失败的bug
          for persona_name, persona in self.personas.items(): 
    ...
    while True: 
      ...
      try: 
        if sim_command.lower() in ["f", "fin", "finish", "save and finish"]: 
          ...
          # Example: exit 
          #下面会把当前模拟的文件删除并退出，但我不希望这样，而是保存并退出
          # shutil.rmtree(sim_folder) 
          self.save()
          break 
